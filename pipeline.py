"""
End-to-end pipeline (free stack: Groq for text, local SpeechT5 for TTS):

  1. Load fluent English sentences (one per line, or a CSV column).
  2. Sample disfluency category/categories per sentence.
  3. Batch-call Groq's free-tier LLM to inject stutter-like disfluencies
     into the text.
  4. Synthesize both the clean and the disfluent text with local SpeechT5,
     matching the paper's speaker/speed sampling as closely as a
...
Run with --dry-run-text to only do step 1-3 (no TTS / no model download)
so you can sanity-check the disfluent text before running SpeechT5.

Note: the first TTS run downloads SpeechT5 + vocoder + speaker embeddings
from HuggingFace (a few hundred MB total) and caches them locally.
"""
...
def run(args):
    ...
    tts = TTSGenerator(seed=args.seed)  # loads SpeechT5 + vocoder + speakers once
    manifest_rows = []
    skipped = 0
    ...
    for idx, item in enumerate(dataset):
        ...
        try:
            ...
            print(f"[{uid}] SKIPPED (TTS failed): {e}")
            skipped += 1
            continue
    ...
    if not manifest_rows:
        print("No audio was successfully generated; manifest not written.")
        return
    ...
    print(f"Wrote manifest -> {MANIFEST_PATH} ({len(manifest_rows)} rows, {skipped} skipped)")
