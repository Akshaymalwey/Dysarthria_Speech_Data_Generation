"""
LLM-based text disfluency injection for English transcripts, using Groq's
free-tier API (OpenAI-compatible chat completions) in place of GPT-4o-mini.
...
class DisfluencyGenerator:
    def __init__(self, api_key: str = None, model: str = TEXT_MODEL):
        # Groq() reads GROQ_API_KEY from the environment if api_key is None.
    ...
    def _call_llm(self, batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for attempt in range(1, TEXT_MAX_RETRIES + 1):
            try:
                ...
                # Some models wrap the array in a top-level object even when
                # not explicitly asked to; handle both shapes defensively.
